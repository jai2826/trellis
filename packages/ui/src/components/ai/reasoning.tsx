...
const AIReasoningContext =
  createContext<AIReasoningContextValue | null>(null);

const useAIReasoning = () => {
  const context = useContext(AIReasoningContext);
  if (!context) {
    throw new Error(
      "AIReasoning components must be used within AIReasoning"
    );
  }
  return context;
};

export type AIReasoningProps = ComponentProps<
  typeof Collapsible
> & {
  isStreaming?: boolean;
  open?: boolean;
...
export const AIReasoning = memo(
  ({
    ...
    const [hasAutoClosedRef, setHasAutoClosedRef] =
      useState(false);
...
export const AIReasoningTrigger = memo(
  ({
    ...
    const { isStreaming, isOpen, duration } =
      useAIReasoning();
...
export const AIReasoningContent = memo(
  ({
    ...
    <CollapsibleContent
      ...
      {...props}>
      <AIResponse className="grid gap-2">
        {children}
      </AIResponse>
    </CollapsibleContent>
  )
